...
export function Explorer({ data }: { data: GraphData }) {
  ...
  const showGlobal = () => { setView("graph"); setGraphScope("global"); };
  const selectTul = (name: string) => { setTul(name); clear(); setView("graph"); setGraphScope("selected"); };

  return <main id="top" className="shell">
    <section className="hero" aria-labelledby="title">
      ...
      <aside className="source-note"><details><summary>Fuente editorial</summary><span>Manual de Teoría y Programa Técnico de Taekwon-Do ITF · Asociación Colombiana de Taekwon-Do ITF</span></details></aside>
    </section>
    ...
    <section className="filter-panel" aria-label="Filtros de exploración">
      ...
      <div className="filter-grid">
        <SelectField label="Figura o tul" value={tul} onChange={(event) => selectTul(event.target.value)}>{data.tules.map((name) => <option key={name}>{name}</option>)}</SelectField>
      ...
      <button type="button" className="mobile-global-link" onClick={showGlobal} aria-pressed={graphScope === "global"}>Ver grafo global</button>
    </section>
    ...
    <section className="workspace">
      <nav className="pattern-nav" aria-label="Ruta pedagógica"><div className="section-label">Ruta pedagógica</div><button className={`pattern-card global-graph-button ${view === "graph" && graphScope === "global" ? "global-graph-button--active" : ""}`} onClick={showGlobal} aria-pressed={view === "graph" && graphScope === "global"}><span>Grafo global</span><span>Vista completa</span></button>{data.tules.map((name) => <PatternCard key={name} name={name} movements={data.movement_sequences[name].length} active={name === tul && graphScope === "selected"} onClick={() => selectTul(name)} />)}</nav>
      <article className="detail-panel">
        <div className="mobile-quickbar">
          <a href="#top" className="quickbar-top" aria-label="Volver arriba">↑</a>
          <select aria-label="Cambiar de figura o tul" value={tul} onChange={(event) => selectTul(event.target.value)}>{data.tules.map((name) => <option key={name}>{name}</option>)}</select>
        </div>
